...
class SqliteBackupService {
  ...
  static SqliteBackupService get instance =>
      _instance ??= SqliteBackupService._();
  ...
  Future<File> createBackup({String? customName}) async {
    ...
    archive.addFile(ArchiveFile(
        'metadata.json', metadataJson.length, metadataJson.codeUnits));
    ...
    debugPrint(
        '[SqliteBackup] Backup created: ${backupFile.path} (${zipBytes.length} bytes)');
  ...
  Future<void> restoreBackup({
    ...
    if (mode == 'replace') {
      ...
      final backupPath = p.join(backupDir.path,
          'database_before_restore_${DateTime.now().millisecondsSinceEpoch}.sqlite');
      if (await dbFile.exists()) {
      ...
      for (final file in archive) {
        if (file.name.endsWith('-wal') || file.name.endsWith('-shm')) {
          final jFile = File(
              '${dbFile.path}${file.name.substring(file.name.lastIndexOf('-'))}');
      ...
      throw Exception(
          'Merge mode not yet implemented. Please use replace mode.');
    }
  }
